document.addEventListener('DOMContentLoaded', () => {
    ...
    async function renderAyahSearchResults(matches) {
        surahListEl.innerHTML = matches.map(match => `
            <div class="surah-card ayah-result" data-surah-num="${match.surah.number}" data-ayah-num="${match.numberInSurah}">
                <div class="number">${match.surah.number}</div>
                <div class="surah-info">
                    <h3>${match.surah.name} (آية ${match.numberInSurah})</h3>
                    <p class="ayah-snippet">${match.text}</p>
                </div>
                <i class="fas fa-play-circle" style="font-size: 1.5rem; color: var(--primary-color);"></i>
            </div>
        `).join('');

        document.querySelectorAll('.ayah-result').forEach(card => {
                ...
                if (surah) {
                    ...
                    if (ayahs) {
                        ayahContent.innerHTML = ayahs.map(a => `
                            <span class="ayah-txt" id="ayah-${a.numberInSurah}">${a.text} <span class="ayah-num">(${a.numberInSurah})</span></span>
                        `).join(' ');
        ...
        navItems.forEach(item => {
            item.addEventListener('click', () => {
                ...
                if (target === 'ai') {
                    ...
                    document.querySelector('.player-bar').style.display = 'none';
                } else {
